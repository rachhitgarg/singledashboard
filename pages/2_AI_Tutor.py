...
# Order phase for consistent plots (no Phase filter in UI)
for df in [sess, util, wk, sumc]:
    phase_order(df)
...
sess_f = apply_filters(sess.copy())
util_f = apply_filters(util.copy())
wk_f = apply_filters(wk.copy())
sumc_f = apply_filters(sumc.copy())
...
if not sumc_f.empty:
    fig, ax = plt.subplots()
    p = sumc_f.groupby("Phase", observed=True)[["PreTutor_Exam_Avg", "PostTutor_Exam_Avg"]].mean()
