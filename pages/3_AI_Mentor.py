...
mc_f = apply_filters(mc.copy())
pc_f = apply_filters(pc.copy())

c1, c2, c3 = st.columns(3)
c1.metric(
    "PostMentor Capstone Avg",
    round(mc_f["PostMentor_Capstone_Grade_Avg"].mean(), 2) if not mc_f.empty else "—",
)
c2.metric(
    "Grade A% (Post)",
    round(mc_f["Grade_A_Distribution_%_Post"].mean(), 2) if not mc_f.empty else "—",
)
c3.metric(
    "Tier-1 Share (%)",
...
if not mc_f.empty:
    fig, ax = plt.subplots()
    tmp = mc_f.groupby("Phase", observed=True)[
