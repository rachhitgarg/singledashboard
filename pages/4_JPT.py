...
c1.metric(
    "Avg JPT Sessions/Student",
    round(jpt_f["Avg_Sessions_Per_Student"].mean(), 2) if not jpt_f.empty else "—",
)
c2.metric(
    "AI Technical (Avg)",
...
c3.metric(
    "Pre Conv/Open (%)",
    round(pd.to_numeric(jpt_f["PreJPT_Conv_Rate_Per_Opening_%"], errors="coerce").mean(), 2)
    if not jpt_f.empty
    else "—",
)
c4.metric(
    "Post Conv/Open (%)",
    round(pd.to_numeric(jpt_f["PostJPT_Conv_Rate_Per_Opening_%"], errors="coerce").mean(), 2)
    if not jpt_f.empty
    else "—",
)
...
if not jpt_f.empty:
    ...
    grp = tmp.groupby("Phase", observed=True)[
        ["PreJPT_Conv_Rate_Per_Opening_%", "PostJPT_Conv_Rate_Per_Opening_%"]
    ].mean().reset_index()
    fig, ax = plt.subplots()
    ...
    ax.set_ylabel("Conversion per Opening (%)")
    ax.legend()
    st.pyplot(fig)
...
if not jpt_f.empty:
    ...
    p = tmp.groupby("Phase", observed=True)[["Tier1_Offers_Before", "Tier1_Offers_After"]].sum()
